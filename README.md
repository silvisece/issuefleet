...
are written to the worker's inbox and injected into its next turn. It answers
feedback where the feedback was left: `agentctl reply` posts into the same
PR/MR thread, or as a PR comment mentioning the reviewer where GitHub has no
thread. Agents commit freely — a linked worktree's objects land in the shared
`.git` on the host — but nothing leaves the machine until the orchestrator
pushes it.
